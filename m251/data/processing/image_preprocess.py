...
Largly copied from https://github.com/google-research/simclr/blob/master/tf2/data_util.py
"""
import functools
...
def batch_random_blur(images_list, height, width, blur_probability=0.5):
    ...
    def generate_selector(p, bsz):
        shape = [bsz, 1, 1, 1]
        selector = tf.cast(
            tf.less(tf.random.uniform(shape, 0, 1, dtype=tf.float32), p), tf.float32
        )
        return selector
...
def preprocess_for_train(
    image, height, width, color_distort=True, crop=True, flip=True, impl="simclrv2"
):
...
def preprocess_image(
    image, height, width, is_training=False, color_distort=True, test_crop=True
):
