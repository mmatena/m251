...
from .fisher3 import Fisher_PretrainMore, Fisher_PretrainMore_REAL
from .fisher3 import Fisher_PretrainFromDapt32768
from .fisher3 import Fisher_Pretrain32768NoReg
...
def _to_mtm(run_params, fishers):
    ...
    model_checkpoint_uuid = getattr(
        run_params, "finetuned_ckpt_uuid", getattr(run_params, "checkpoint", None)
    )
    return ModelToMerge(
...
@experiment.experiment(
    uuid="41aea56f354b419996039fcf7c2bd5ad",
    group=PaperExpGroup,
    ...
    varying_params=functools.partial(
        ...
        donor_fisher_exp=Fisher_PretrainMore,
    ),
    fixed_params={
        "num_weightings": 76,
        #
...
class Merge_ROBERTA_LastCkpt_TestSet_PretrainedMore(ExperimentAbc):
    # NOTE: Actually just 32k mlm examples as I messed up in the Fisher experiments.
    pass


@experiment.experiment(
    uuid="a193a43770774310b4aea580a10d466a",
    group=PaperExpGroup,
    params_cls=MergeParams,
    executable_cls=merging_execs.merge_and_evaluate_from_checkpoints,
    varying_params=functools.partial(
        create_varying_params,
        target_fisher_exp=FisherComputation_ROBERTA_TargetTasks_LastCkpt_AllVars,
        donor_fisher_exp=Fisher_PretrainMore_REAL,
    ),
    fixed_params={
        "num_weightings": 76,
        #
...
class Merge_ROBERTA_LastCkpt_TestSet_PretrainedMore_REAL(ExperimentAbc):
    pass


@experiment.experiment(
    uuid="5802711888fc40aca90fa3ecc0f4b8cb",
    group=PaperExpGroup,
    params_cls=MergeParams,
    executable_cls=merging_execs.merge_and_evaluate_from_checkpoints,
    varying_params=functools.partial(
...
class Merge_ROBERTA_LastCkpt_TestSet_PretrainFromDapt32768(ExperimentAbc):
    pass


@experiment.experiment(
    uuid="2cfa90071baa475e9f3251bd2a6341be",
    group=PaperExpGroup,
    params_cls=MergeParams,
    executable_cls=merging_execs.merge_and_evaluate_from_checkpoints,
    varying_params=functools.partial(
        create_varying_params,
        target_fisher_exp=FisherComputation_ROBERTA_TargetTasks_LastCkpt_AllVars,
        donor_fisher_exp=Fisher_Pretrain32768NoReg,
    ),
    fixed_params={
        "num_weightings": 76,
        #
...
class Merge_ROBERTA_LastCkpt_TestSet_Pretrain32768NoReg(ExperimentAbc):
    pass
