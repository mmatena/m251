...
from .fisher import DirectFisherComputation, VariationalFisherComputation
...
class VariationalMergeParams(MergeParamsAbc):
    def __init__(
        self,
        #
        trial_index,
    ...
    def create_bindings(self):
        return {
            "mergeable_model": vardiag_execs.diagonal_mergeable_model_from_checkpoint,
...
@experiment.experiment(
    uuid="16376b6ca7454cb68a12c9b0dc2f5410",
    group=PaperExpGroup,
    params_cls=VariationalMergeParams,
    executable_cls=merging_execs.merge_and_evaluate_from_checkpoints,
    varying_params=functools.partial(
        create_varying_params,
        fisher_exp=VariationalFisherComputation,
    ),
...
class MergeVariationalFishers(ExperimentAbc):
    pass


@experiment.experiment(
    uuid="a9bb940ccce841a195d042fd0e298b03",
    group=PaperExpGroup,
    params_cls=DirectMergeParams,
    executable_cls=merging_execs.merge_and_evaluate_from_checkpoints,
    varying_params=functools.partial(
        create_varying_params,
        fisher_exp=DirectFisherComputation,
    ),
    fixed_params=FIXED_PARAMS,
    key_fields=KEY_FIELDS,
    bindings=COMMON_BINDINGS
