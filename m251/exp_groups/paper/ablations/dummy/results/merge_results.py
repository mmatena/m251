...
python3 m251/exp_groups/paper/ablations/dummy/results/merge_results.py
...
MERGE_ID_FISHER_JSON = result_file("ablations/dummy/merge_id_fisher.json")
MERGE_SHUFFLED_FISHER_JSON = result_file("ablations/dummy/merge_shuffled_fisher.json")
...
def create_csv_table(filepath, round_digits=1):
    ...
    for item in items:
        if item["hyperparams"]["target_ckpt_index"] != 9:
            continue
        elif item["hyperparams"]["donor_ckpt_index"] != 7:
            continue
...
if __name__ == "__main__":
    from m251.exp_groups.paper.ablations.dummy import merge

    ###########################################################################

    merge_exp = merge.Merge_Shuffled
    summary = create_json(merge_exp)
    # s = json.dumps(summary, indent=2)
    # print(s)
    filepath = MERGE_SHUFFLED_FISHER_JSON
    ...
    # t = create_csv_table(MERGE_RTE_10_EPOCHS_JSON)
    # print(t)
