...
def create_varying_params(exp, train_exp, target_fisher_exp, donor_fisher_exp):
    train_run_uuids = train_exp.retrieve_run_uuids(RunState.FINISHED)
    target_fisher_run_uuids = target_fisher_exp.retrieve_run_uuids(RunState.FINISHED)
    donor_fisher_run_uuids = donor_fisher_exp.retrieve_run_uuids(RunState.FINISHED)

    train_run_params = {
        rid: train_exp.retrieve_run_params(rid) for rid in train_run_uuids
    }
    target_fisher_run_params = [
        target_fisher_exp.retrieve_run_params(rid) for rid in target_fisher_run_uuids
    ]
    donor_fisher_run_params = [
        donor_fisher_exp.retrieve_run_params(rid) for rid in donor_fisher_run_uuids
    ]

    fisher_run_uuids = target_fisher_run_uuids + donor_fisher_run_uuids
    fisher_run_params = target_fisher_run_params + donor_fisher_run_params
    ...
    for fi_rid, fi_rp in zip(fisher_run_uuids, fisher_run_params):
        ...
        is_target = fi_rid in target_fisher_run_uuids
        fisher_exp = target_fisher_exp if is_target else donor_fisher_exp
...
@experiment.experiment(
    uuid="0fed5f1628174ca5b1a378a8cf37de8f",
    ...
    varying_params=functools.partial(
        ...
        target_fisher_exp=bert_base_fisher_var_diag.RteBestCkpt_Iso_0003_PhaseI,
        donor_fisher_exp=bert_base_fisher_var_diag.MnliBestCkpt_Iso_0003_PhaseI,
    ),
    fixed_params={
...
class RteMnli_BestCkpt_Iso_0003_MergeSame_PhaseI(object):
