...
def create_varying_params_all_mnli_to_mrpc(
    exp, mnli_fisher_exp, mrpc_fisher_exp, train_exp
):
    ...
    mrpc_fisher_run_uuids = mrpc_fisher_exp.retrieve_run_uuids(RunState.FINISHED)
    ...
    mrpc_fisher_run_params = [
        mrpc_fisher_exp.retrieve_run_params(rid) for rid in mrpc_fisher_run_uuids
    ]

    grouping_to_params = collections.defaultdict(list)
    for fi_rid, fi_rp in zip(
        mnli_fisher_run_uuids + mrpc_fisher_run_uuids,
        mnli_fisher_run_params + mrpc_fisher_run_params,
    ):
        assert fi_rp.ft_exp_uuid == train_exp.uuid

        if fi_rp.task not in ["mnli", "mrpc"]:
            continue
    ...
    for grouping_key, models in grouping_to_params.items():
        ...
        (mrpc_model,) = [mtm for mtm in models if mtm.task == "mrpc"]
        ...
        for model in mnli_models:
            ...
            varying_param.update(
                {
                    "models_to_merge": [mrpc_model, model],
                    "task": "mrpc",
                }
            )
...
@experiment.experiment(
    uuid="8d86636fb26c4de995d45d4e916434c0",
    ...
    varying_params=functools.partial(
        create_varying_params_all_mnli_to_mrpc,
        train_exp=finetune_bert_base.Glue_Regs,
        mrpc_fisher_exp=fisher_bert_base.GlueRegs_Fisher_BestCkpt,
...
class MergeBestMrpcWithEachMnli_GlueRegs(object):
